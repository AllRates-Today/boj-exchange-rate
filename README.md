...
[![zero dependencies](https://img.shields.io/badge/dependencies-0-brightgreen.svg)](https://www.npmjs.com/package/boj-exchange-rate)
[![TypeScript](https://img.shields.io/badge/TypeScript-types%20included-3178C6.svg)](https://www.typescriptlang.org/)

**Official Bank of Japan (Japan) daily exchange rates for Node.js and TypeScript. The published central bank rates behind tax filings, customs valuations, audits, and compliant invoicing — not market estimates, but the numbers Bank of Japan itself prints, every business day.**

## 🚀 Why this client?

- 🏛️ **Official published rates** — Bank of Japan's own table, with the publisher's own `rate_date` on every response
...
```bash
npm install boj-exchange-rate
```

```bash
yarn add boj-exchange-rate
```
...
```js
import { getRate } from 'boj-exchange-rate';

const pair = await getRate('USD', 'JPY', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // the official Bank of Japan rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'boj-exchange-rate';
...
```js
import { getRatesForDate } from 'boj-exchange-rate';
...
| `cross` | `true` | Computed via JPY from two published rates |
...
| `404` | Pair or date range not covered by Bank of Japan |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'boj-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('boj-exchange-rate');
